...
// ── Composant image avec fallback emoji ──────────────────────────────────────
...
const ImgWithFallback = ({ src, alt, emoji, className = '' }: ImgWithFallbackProps) => {
...
const FoodDatabase = () => {
  ...
  useEffect(() => {
    if (recipeCategoryFilter) {
      setView('recipes');
      setRecipeCategory(recipeCategoryFilter);
    }
  ...
  const filteredFoods = foodItems.filter(f => {
    ...
    const matchCat =
      category === t.food.categories[0] ||
      f.category === category ||
      (category === t.food.categories[6] && f.glutenFree);
    return matchSearch && matchCat;
  });

  // ── Catégories recettes (dynamiques selon les données) ──
  const allRecipeCategories = Array.from(new Set(recipes.map(r => r.category)));

  // ── Filtres recettes ──
  const filteredRecipes = recipes.filter(r => {
    const matchSearch = r.name.toLowerCase().includes(search.toLowerCase());
    const activeCat = recipeCategory || recipeCategoryFilter;
    const matchCat = activeCat ? r.category === activeCat : true;
    return matchSearch && matchCat;
  ...
  const giColor = (gi: number) =>
    gi <= 35 ? 'text-success' : gi <= 55 ? 'text-warning' : 'text-destructive';

  const giBg = (gi: number) =>
    gi <= 35 ? 'bg-success/10' : gi <= 55 ? 'bg-warning/10' : 'bg-destructive/10';
        ...
        <div className="flex gap-1 mb-5 p-1 bg-muted rounded-2xl">
          {(['foods', 'recipes'] as const).map(v => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => setView(v)}
              className={`relative flex-1 py-2.5 rounded-xl text-sm font-medium transition-all ${view === v ? 'text-primary-foreground' : 'text-muted-foreground'
                }`}
            >
              {view === v && (
                <motion.div
              ...
              <span className="relative z-10">
                {v === 'foods' ? t.food.foods : t.food.recipes}
              </span>
            </motion.button>
          ))}
        </div>
          ...
          {view === 'foods' ? (
            ...
            >
              ...
              <div className="flex gap-2 overflow-x-auto pb-3 mb-4 scrollbar-hide">
                {t.food.categories.map((cat: string, i: number) => (
                  <motion.button
                    key={i}
                    whileTap={{ scale: 0.9 }}
                    onClick={() => setCategory(cat)}
                    className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${category === cat
              ...
              <div className="grid grid-cols-2 gap-3">
                {filteredFoods.map((food, i) => (
                  <motion.button
                    ...
                    className="p-0 rounded-2xl bg-card card-shadow text-left hover:shadow-md transition-shadow overflow-hidden flex flex-col"
                  >
                    <div className="relative h-24 bg-muted overflow-hidden flex-shrink-0">
                      <ImgWithFallback src={food.imageUrl} alt={food.name} emoji={food.emoji} />
                    </div>
                    <div className="p-3 flex flex-col flex-1">
                      <p className={`text-sm font-semibold text-foreground mb-1 line-clamp-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                        {food.name}
                      </p>
                      ...
                      {food.gi > 0 && (
                        <span className={`inline-block mt-2 text-[10px] font-semibold px-2 py-0.5 rounded-full self-start ${giColor(food.gi)} ${giBg(food.gi)}`}>
            ...
            <motion.div
              ...
              transition={{ duration: 0.25 }}
            >
              {/* Bandeau filtre journal */}
              {(recipeCategory || recipeCategoryFilter) && (
                <div className={`bg-primary/10 border border-primary/20 p-3 rounded-2xl mb-4 flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                  <span className="text-sm font-semibold text-primary">
                    {isRTL
                      ? `اقتراحات: ${recipeCategory || recipeCategoryFilter}`
                      : `Suggestions pour : ${recipeCategory || recipeCategoryFilter}`}
                  </span>
                  <button
                    onClick={clearRecipeFilter}
                    className="p-1 text-primary bg-primary/20 rounded-full hover:bg-primary/30 transition-colors"
                  >
                    <X size={14} />
                  </button>
                </div>
              ...
              {/* Filtres catégories recettes */}
              <div className="flex gap-2 overflow-x-auto pb-3 mb-4 scrollbar-hide">
                <motion.button
                  whileTap={{ scale: 0.9 }}
                  onClick={() => setRecipeCategory(null)}
                  className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${!recipeCategory
                      ? 'bg-primary text-primary-foreground shadow-sm'
                      : 'bg-card text-muted-foreground card-shadow hover:bg-muted'
                    }`}
                >
                ...
                {allRecipeCategories.map((cat, i) => (
                  <motion.button
                    key={i}
                    whileTap={{ scale: 0.9 }}
                    onClick={() => setRecipeCategory(cat)}
                    className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${recipeCategory === cat
                        ? 'bg-primary text-primary-foreground shadow-sm'
                        : 'bg-card text-muted-foreground card-shadow hover:bg-muted'
                      }`}
                  >
              ...
              <div className={recipeLayout === 'grid' ? 'grid grid-cols-2 gap-3' : 'flex flex-col gap-3'}>
                {filteredRecipes.map((recipe, i) => (
                  <motion.button
                    ...
                    className={`p-0 rounded-2xl bg-card card-shadow overflow-hidden text-left hover:shadow-md transition-shadow ${recipeLayout === 'grid' ? 'flex flex-col' : 'flex items-center h-28'
                      }`}
                  >
                    <div className={`relative bg-muted flex-shrink-0 overflow-hidden ${recipeLayout === 'grid' ? 'w-full h-32' : 'w-28 h-full'
                      }`}>
                      <ImgWithFallback src={recipe.imageUrl} alt={recipe.name} emoji={recipe.emoji} />
                    ...
                    <div className={`p-3 w-full flex-1 flex flex-col justify-center ${isRTL ? 'text-right' : 'text-left'}`}>
                      ...
                      <div className={`flex flex-wrap gap-x-2 gap-y-1 mt-1.5 text-[10px] text-muted-foreground ${isRTL ? 'justify-end' : 'justify-start'}`}>
                        <span className="flex items-center gap-0.5">
                          <Wheat size={10} className="text-secondary" /> {recipe.carbs}g
                        </span>
                        <span className="flex items-center gap-0.5">
                          <Flame size={10} className="text-orange-500" /> {recipe.calories} {t.food.kcal}
                        </span>
                      </div>
                    </div>
                  </motion.button>
              ...
              {filteredRecipes.length === 0 && (
                <div className="text-center py-12 text-muted-foreground text-sm">
                  {isRTL ? 'لا توجد وصفات.' : 'Aucune recette trouvée.'}
                </div>
              )}
            </motion.div>
          )}
          ...
          {selectedFood && (
            <div className="flex flex-col relative">
              ...
              <div className="relative h-48 bg-muted flex-shrink-0">
                <ImgWithFallback src={selectedFood.imageUrl} alt={selectedFood.name} emoji={selectedFood.emoji} />
                <div className="absolute inset-0 bg-gradient-to-t from-background via-background/20 to-transparent" />
                <div className="absolute bottom-4 left-4 right-4">
                  <h2 className="text-xl font-bold text-foreground mb-1">{selectedFood.name}</h2>
                  <p className="text-sm text-muted-foreground font-medium">
                    {t.food.portion} {selectedFood.portion}
                  </p>
                </div>
              </div>
              <div className="p-6 flex flex-col gap-4">
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: t.food.carbs, value: `${selectedFood.carbs}g`, accent: 'text-blue-500' },
                    { label: t.food.protein, value: `${selectedFood.protein}g`, accent: 'text-emerald-500' },
                    { label: t.food.fat, value: `${selectedFood.fat}g`, accent: 'text-orange-400' },
                ...
                {selectedFood.glutenFree && (
                  <span className="text-xs px-3 py-1.5 rounded-full bg-success/10 text-success self-center font-medium">
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto rounded-3xl p-0 hide-scrollbar shadow-2xl border-0">
          ...
          {selectedRecipe && (
            <div className="flex flex-col relative w-full pb-4">
              ...
              {/* Image recette */}
              <div className="relative w-full h-[200px] bg-muted flex-shrink-0 overflow-hidden">
                <ImgWithFallback
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  ...
                  <div className="flex items-center gap-3 text-xs font-medium text-muted-foreground">
                    <span className="flex items-center gap-1 bg-background/60 backdrop-blur-md px-2 py-1 rounded-md text-primary">
                      <Clock size={12} /> {selectedRecipe.time} {t.food.time}
                    </span>
                    <span className="flex items-center gap-1 bg-background/60 backdrop-blur-md px-2 py-1 rounded-md">
              ...
              <div className="px-6 py-4 flex flex-col gap-5">
                <div className="flex justify-between p-4 rounded-2xl bg-muted/30 border border-border/50">
                  {[
                    { label: t.food.carbs, value: `${selectedRecipe.carbs}g`, color: 'text-blue-500' },
                    { label: t.food.protein, value: `${selectedRecipe.protein}g`, color: 'text-emerald-500' },
                    { label: t.food.fat, value: `${selectedRecipe.fat || 0}g`, color: 'text-orange-400' },
                  ].map((m, i, arr) => (
                    <div key={i} className={`text-center flex-1 ${i < arr.length - 1 ? 'border-r border-border/50' : ''}`}>
                      <span className="text-[11px] text-muted-foreground font-medium uppercase mb-1 block">
                        {m.label}
                      </span>
                ...
                {/* Ingrédients */}
                <div>
                  <h4 className={`font-bold text-foreground mb-3 text-sm flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <span className="w-6 h-6 rounded-full bg-secondary/10 text-secondary flex items-center justify-center text-xs">
                      📝
                    </span>
                    {t.food.ingredients}
                  </h4>
                  <ul className="text-sm text-muted-foreground space-y-2">
                    {selectedRecipe.ingredients.map((ing, i) => (
                      <li
                        key={i}
                        className={`flex items-center gap-3 p-2.5 rounded-xl bg-card border border-border/30 ${isRTL ? 'flex-row-reverse' : ''}`}
                      >
                ...
                {/* Instructions */}
                <div className="mb-2">
                  <h4 className={`font-bold text-foreground mb-4 text-sm flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <span className="w-7 h-7 rounded-full bg-primary/10 text-primary flex items-center justify-center text-sm">
                      👨‍🍳
                    </span>
                    {t.food.instructions}
                  </h4>
                  <ol className={`text-sm text-muted-foreground space-y-4 relative before:absolute before:top-2 before:bottom-2 before:w-0.5 before:bg-border/60 ${isRTL ? 'before:right-3.5' : 'before:left-3.5'}`}>
                    {selectedRecipe.instructions.map((step, i) => (
                      <li
                        key={i}
                        className={`flex gap-4 relative ${isRTL ? 'flex-row-reverse' : ''}`}
                      >
                ...
                <div className="pt-2">
                  <Button
                    onClick={() => {
                      toast.success(
                        isRTL ? 'تمت الإضافة إلى اليوميات!' : 'Ajouté au journal du jour !',
                      );
